...
from AnonXMusic.utils.sqlite_helper import Cache
...
async def check_perms(
    ...
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR:
        if complain_missing_perms:
            await sender("You need to be an admin to perform this action.")
        return False
    ...
    if complain_missing_perms:
        await sender(
            "You lack the following permissions: {}".format(", ".join(missing_perms))
        )
    return False
    ...
    def decorator(func):
        ...
        async def wrapper(
            ...
            if msg.chat.type == enums.ChatType.PRIVATE:
                ...
                return await sender("This action is not allowed in private chats.")
            if msg.chat.type == enums.ChatType.CHANNEL:
                return await func(client, message, *args, **kwargs)
            has_perms = await check_perms(
                message, permissions, complain_missing_perms
            )
            if has_perms:
                return await func(client, message, *args, **kwargs)
