...
async def tgs_to_gif(file, tgs=False, video=False):
    if tgs:
        cmd = f"lottie_convert.py '{file}' 'app_satoru.gif'"
    elif video:
        cmd = f"ffmpeg -i '{file}' -c copy 'app_satoru.gif'"
    await runcmd(cmd)
    os.remove(file)
    return 'asuna.gif'

async def webm_to_gif(file):
    cmd = f"ffmpeg -i '{file}' 'app.gif'"
    await runcmd(cmd)
    os.remove(file)
    return "asuna.gif"
        
async def Vsticker(c: app, file: Message):
    if file.animation:
        file = file.animation
    elif file.video:
        file = file.video
...
async def upload_document(
    client: app, file_path: str, chat_id: int
...
async def draw_meme(image_path: str, text: str, sticker: bool, fiill: str) -> list:
    ...
    font = ImageFont.truetype("./AnonXMusic/assets/comic.ttf", font_size)

    draw = ImageDraw.Draw(image)

    # Calculate text dimensions using textbbox
    upper_text_bbox = draw.textbbox((0, 0), upper_text, font=font)
    upper_text_width = upper_text_bbox[2] - upper_text_bbox[0]

    lower_text_bbox = draw.textbbox((0, 0), lower_text, font=font)
    lower_text_width = lower_text_bbox[2] - lower_text_bbox[0]
    lower_text_height = lower_text_bbox[3] - lower_text_bbox[1]
    ...
    image1 = image_path
    image2 = tosticker(stick_path, "@asuna_memer_hu_vai.webp")
...
def toimage(image, filename=None, is_direc=False):
    filename = filename if filename else "asuna.jpg"
    if is_direc:
...
def tosticker(response, filename=None, is_direc=False):
    filename = filename if filename else "asuna.webp"
    if is_direc:
