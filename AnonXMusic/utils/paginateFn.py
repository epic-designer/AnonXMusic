...
def to_styled_font(text):
    ...
    small_caps = "ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴘϙʀsᴛᴜᴠᴡxʏᴢ"  # Corresponding small caps
...
def paginate_modules(page, module_dict, prefix):
    ...
    pairs = [modules[i * 12:(i + 1) * 12] for i in range((len(modules) + 12 - 1) // 12)]
    ...
    if len(pairs) > 1:
        buttons.append([
            InlineKeyboardButton("☜", callback_data=f"{prefix}prev({modulo_page - 1})"),  # Previous page button
            InlineKeyboardButton("𝙃𝙊𝙈𝙀", callback_data="settingsback_helper"),  # Home button
            InlineKeyboardButton("☞", callback_data=f"{prefix}next({modulo_page + 1})")  # Next page button
        ])
    else:
        buttons.append([InlineKeyboardButton("𝙃𝙊𝙈𝙀", callback_data="settingsback_helper")])  # Only Home button if single page
