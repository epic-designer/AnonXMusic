...
from AnonXMusic.utils.http_helper import *
from AnonXMusic.utils.parser import mention_html


@app.on_message(command("wiki"))
...
async def gdpr_remove(_, m: Message):
    ...
    await m.reply_text(
        ...
        "Note that this will not unban you from any chats, as that is telegram data, not app data."
...
async def id_info(c: app, m: Message):
...
async def get_gifid(_, m: Message):
    if m.reply_to_message and m.reply_to_message.animation:
        LOGGER(__name__).info(f"{m.from_user.id} used gifid cmd in {m.chat.id}")
        await m.reply_text(
...
async def github(_, m: Message):
    if len(m.text.split()) == 2:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} used github cmd in {m.chat.id}")
    else:
        await m.reply_text(
...
async def reporting_query(c: app, m: Message):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("Update channel",url=f"https://t.me/{SUPPORT_GROUP}")],[InlineKeyboardButton("Report on github",url="https://github.com/app-Bots/app_Satoru/issues/new/choose")]])
    try:
...
**Example:**
`/git username`: this fetches the information about a user from the database."""
