...
from AnonXMusic.utils.cmd_senders import send_cmd
from AnonXMusic.utils.custom_filters import admin_filter, bot_admin_filter, command
from AnonXMusic.utils.kbhelpers import ikb
from AnonXMusic.utils.msg_types import Types, get_wlcm_type
from AnonXMusic.utils.parser import escape_markdown, mention_html
from AnonXMusic.utils.string import (build_keyboard, escape_invalid_curly_brackets,
                                 parse_button)
import config
...
@app.on_message(command("setwelcome") & admin_filter & bot_admin_filter)
...
@app.on_message(command("setgoodbye") & admin_filter & bot_admin_filter)
@capture_err
...
@app.on_message(command("resetgoodbye") & admin_filter & bot_admin_filter)
@capture_err
...
@app.on_message(command("resetwelcome") & admin_filter & bot_admin_filter)
@capture_err
...
async def member_has_joined(c: app, member: ChatMemberUpdated):
    ...
    try:
        ...
        if user.id in DEV_USERS:
            await c.send_animation(
                ...
                animation="http://telegra.ph/file/c072df96e61500c399a87.mp4",
    ...
    if status:
        ...
        try:
            ...
            LOGGER(__name__).error(format_exc(e))
            return
    else:
        return
...
× bot must be an admin to greet and goodbye users.
