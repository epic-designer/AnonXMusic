...
from AnonXMusic import HELP_COMMANDS, LOGGER, app
from AnonXMusic.database.disable_db import Disabling
from AnonXMusic.utils.custom_filters import (admin_filter, can_change_filter,
                                         command, owner_filter)


@app.on_message(command("disable") & can_change_filter)
async def disableit(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} used disabled cmd in {m.chat.id}")
...
@app.on_message(command("disabledel") & can_change_filter)
async def set_dsbl_action(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} disabledel used in {m.chat.id}")
...
@app.on_message(command("enable") & can_change_filter)
async def enableit(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} enabled something in {m.chat.id}")
    return await m.reply_text(f"Enabled {m.text.split(None, 1)[1]}!")


@app.on_message(command("disableable") & can_change_filter)
async def disabling(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} checked disableable {m.chat.id}")
    return await m.reply_text(tes)


@app.on_message(command("disabled") & admin_filter)
async def disabled(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} checked disabled {m.chat.id}")
    return await m.reply_text(tex)


@app.on_message(command("enableall") & owner_filter)
...
async def enablealll(_, q: CallbackQuery):
    ...
    db.rm_all_disabled()
    LOGGER(__name__).info(f"{user_id} enabled all in {q.message.chat.id}")
