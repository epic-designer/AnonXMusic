...
from AnonXMusic.utils.custom_filters import admin_filter, command
from AnonXMusic.utils.kbhelpers import ikb
from AnonXMusic.utils.string import build_keyboard, parse_button
...
async def unpinall_calllback(c: app, q: CallbackQuery):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{q.from_user.id} unpinned all messages in {q.message.chat.id}")
...
async def anti_channel_pin(_, m: Message):
    ...
    if len(m.text.split()) == 2:
        if m.command[1] in ("yes", "on", "true"):
            ...
            LOGGER(__name__).info(f"{m.from_user.id} enabled antichannelpin in {m.chat.id}")
            ...
            LOGGER(__name__).info(f"{m.from_user.id} disabled antichannelpin in {m.chat.id}")
...
async def clean_linked(_, m: Message):
    ...
    if len(m.text.split()) == 2:
        if m.command[1] in ("yes", "on", "true"):
            ...
            LOGGER(__name__).info(f"{m.from_user.id} enabled CleanLinked in {m.chat.id}")
            msg = "Turned on CleanLinked! Now all the messages from linked channel will be deleted!"
        elif m.command[1] in ("no", "off", "false"):
            pinsdb.cleanlinked_off()
            LOGGER(__name__).info(f"{m.from_user.id} disabled CleanLinked in {m.chat.id}")
...
async def perma_pin(_, m: Message):
    if m.reply_to_message or len(m.text.split()) > 1:
        LOGGER(__name__).info(f"{m.from_user.id} used permampin in {m.chat.id}")
        if m.reply_to_message:
