...
from AnonXMusic.utils.custom_filters import command, owner_filter, restrict_filter
...
async def view_blacklist(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} checking blacklists in {m.chat.id}")
...
async def add_blacklist(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} added new blacklists ({bl_words}) in {m.chat.id}")
...
async def rm_blacklist(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} removed blacklists ({bl_words}) in {m.chat.id}")
...
async def set_bl_action(_, m: Message):
    ...
    if len(m.text.split()) == 2:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} checking blacklist action in {m.chat.id}")
        await m.reply_text(
...
async def rm_allbl_callback(_, q: CallbackQuery):
    ...
    LOGGER(__name__).info(f"{user_id} removed all blacklists in {q.message.chat.id}")
