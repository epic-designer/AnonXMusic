...
async def approve_user(c: app, m: Message):
    ...
    LOGGER(__name__).info(f"{user_id} approved by {m.from_user.id} in {m.chat.id}")
...
async def disapprove_user(c: app, m: Message):
    ...
    try:
        ...
        if already_approved:  # If user is approved and not in chat, unapprove them.
            ...
            LOGGER(__name__).info(f"{user_id} disapproved in {m.chat.id} as UserNotParticipant")
    ...
    db.remove_approve(user_id)
    LOGGER(__name__).info(f"{user_id} disapproved by {m.from_user.id} in {m.chat.id}")
...
@app.on_message(command("approved") & admin_filter)
async def check_approved(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} checking approved users in {m.chat.id}")
    return


@app.on_message(command("approval") & filters.group)
async def check_approval(c: app, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} checking approval of {user_id} in {m.chat.id}")
...
async def unapproveall_callback(_, q: CallbackQuery):
    ...
    LOGGER(__name__).info(f"{user_id} disapproved all users in {q.message.chat.id}")
