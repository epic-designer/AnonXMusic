...
from AnonXMusic.utils.caching import ADMIN_CACHE, admin_cache_reload
from AnonXMusic.utils.custom_filters import command, restrict_filter
from AnonXMusic.utils.extract_user import extract_user
from AnonXMusic.utils.extras import BAN_GIFS, KICK_GIFS
from AnonXMusic.utils.parser import mention_html
from AnonXMusic.utils.string import extract_time
...
async def tban_usr(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} tbanned {user_id} in {m.chat.id}")
        ...
        try:
            ...
            await c.send_message(LOGGER_ID,f"#REMOVE from BAN_GFIS\n{anim}")
...
async def stban_usr(c: app, m: Message):
    ...
    try:
        LOGGER(__name__).info(f"{m.from_user.id} stbanned {user_id} in {m.chat.id}")
...
async def dtban_usr(c: app, m: Message):
    ...
    try:
        ...
        banned = await mention_html(user_first_name, user_id)
        chat_title = m.chat.title
        LOGGER(__name__).info(f"{m.from_user.id} dtbanned {user_id} in {m.chat.id}")
        await m.chat.ban_member(user_id, until_date=bantime)
        await m.reply_to_message.delete()
...
async def kick_usr(c: app, m: Message):
    ...
    try:
        ...
        chat_title = m.chat.title
        LOGGER(__name__).info(f"{m.from_user.id} kicked {user_id} in {m.chat.id}")
        ...
        try:
            ...
            await c.send_message(LOGGER_ID,f"#REMOVE from KICK_GFIS\n{kickk}")
...
async def skick_usr(c: app, m: Message):
    ...
    try:
        LOGGER(__name__).info(f"{m.from_user.id} skicked {user_id} in {m.chat.id}")
        await m.chat.ban_member(user_id)
        await m.delete()
        if m.reply_to_message:
            await m.reply_to_message.delete()
...
async def dkick_usr(c: app, m: Message):
    ...
    try:
        LOGGER(__name__).info(f"{m.from_user.id} dkicked {user_id} in {m.chat.id}")
        await m.reply_to_message.delete()
        await m.chat.ban_member(user_id)
        admin = await mention_html(m.from_user.first_name, m.from_user.id)
        kicked = await mention_html(user_first_name, user_id)
...
async def sban_usr(c: app, m: Message):
    ...
    try:
        LOGGER(__name__).info(f"{m.from_user.id} sbanned {user_id} in {m.chat.id}")
        await m.chat.ban_member(user_id)
        await m.delete()
        if m.reply_to_message:
            await m.reply_to_message.delete()
...
async def dban_usr(c: app, m: Message):
    ...
    try:
        LOGGER(__name__).info(f"{m.from_user.id} dbanned {user_id} in {m.chat.id}")
        await m.reply_to_message.delete()
        await m.chat.ban_member(user_id)
        ...
        try:
            ...
            await c.send_messagea(LOGGER_ID,f"#REMOVE from BAN_GIFS\n{animm}")
    except ChatAdminRequired:
        await m.reply_text(text="I'm not admin or I don't have rights.")
    except PeerIdInvalid:
        await m.reply_text(
...
async def ban_usr(c: app, m: Message):
    ...
    try:
        LOGGER(__name__).info(f"{m.from_user.id} banned {user_id} in {m.chat.id}")
        await m.chat.ban_member(user_id)
        banned = await mention_html(user_first_name, user_id)
...
async def kickme(c: app, m: Message):
    reason = None
    if len(m.text.split()) >= 2:
        reason = m.text.split(None, 1)[1]
    try:
        LOGGER(__name__).info(f"{m.from_user.id} kickme used by {m.from_user.id} in {m.chat.id}")
