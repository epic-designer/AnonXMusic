...
from AnonXMusic.utils.extras import MUTE_GIFS
...
async def tmute_usr(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} tmuted {user_id} in {m.chat.id}")
        ...
        try:
            ...
            await c.send_message(Config.LOGGER_ID,f"#REMOVE from MUTE_GIFS\n{mutt}")
...
async def dtmute_usr(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} dtmuted {user_id} in {m.chat.id}")
...
async def stmute_usr(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} stmuted {user_id} in {m.chat.id}")
        await m.delete()
...
async def mute_usr(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} muted {user_id} in {m.chat.id}")
        admin = await mention_html(m.from_user.first_name, m.from_user.id)
        muted = await mention_html(user_first_name, user_id)
        txt = f"Admin {admin} muted {muted}!"
        if reason:
...
async def smute_usr(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} smuted {user_id} in {m.chat.id}")
        await m.delete()
        if m.reply_to_message:
            await m.reply_to_message.delete()
            return
...
async def dmute_usr(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} dmuted {user_id} in {m.chat.id}")
        await m.reply_to_message.delete()
        admin = await mention_html(m.from_user.first_name, m.from_user.id)
        muted = await mention_html(user_first_name, user_id)
        txt = f"Admin {admin} muted {muted}!"
...
async def unmute_usr(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).exception(format_exc())
    try:
        await m.chat.unban_member(user_id)
        LOGGER(__name__).info(f"{m.from_user.id} unmuted {user_id} in {m.chat.id}")
        admin = await mention_html(m.from_user.first_name, m.from_user.id)
...
async def unmutebutton(c: app, q: CallbackQuery):
    ...
    if not user.privileges.can_restrict_members and user.id != Config.OWNER_ID:
        await q.answer(
            "You don't have enough permission to do this!\nStay in your limits!",
            show_alert=True,
        )
