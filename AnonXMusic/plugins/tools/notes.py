...
from AnonXMusic.utils.custom_filters import admin_filter, command, owner_filter
from AnonXMusic.utils.kbhelpers import ikb
from AnonXMusic.utils.msg_types import Types, get_note_type
from AnonXMusic.utils.string import (build_keyboard,
...
async def save_note(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} saved note ({note_name}) in {m.chat.id}")
    await m.reply_text(
...
async def priv_notes(_, m: Message):
    ...
    if len(m.text.split()) == 2:
        ...
        if option in ("on", "yes"):
            ...
            LOGGER(__name__).info(f"{m.from_user.id} enabled privatenotes in {m.chat.id}")
            msg = "Set private notes to On"
        elif option in ("off", "no"):
            db_settings.set_privatenotes(chat_id, False)
            LOGGER(__name__).info(f"{m.from_user.id} disabled privatenotes in {m.chat.id}")
        ...
        LOGGER(__name__).info(f"{m.from_user.id} fetched privatenotes preference in {m.chat.id}")
        await m.reply_text(msg)
    else:
...
async def local_notes(_, m: Message):
    LOGGER(__name__).info(f"{m.from_user.id} listed all notes in {m.chat.id}")
...
async def clear_note(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} cleared note ({note}) in {m.chat.id}")
    if not getnote:
...
async def clearallnotes_callback(_, q: CallbackQuery):
    ...
    LOGGER(__name__).info(f"{user_id} removed all notes in {q.message.chat.id}")
