...
from AnonXMusic.utils.msg_types import Types, get_filter_type
...
async def view_filters(_, m: Message):
    LOGGER(__name__).info(f"{m.from_user.id} checking filters in {m.chat.id}")
...
async def add_filter(_, m: Message):
    ...
    LOGGER(__name__).info(f"{m.from_user.id} added new filter ({keyword}) in {m.chat.id}")
    if add:
        await m.reply_text(
...
async def stop_filter(_, m: Message):
    ...
    for keyword in act_filters:
        if keyword == m.text.split(None, 1)[1].lower():
            ...
            LOGGER(__name__).info(f"{m.from_user.id} removed filter ({keyword}) in {m.chat.id}")
            await m.reply_text(
...
async def rm_allfilters_callback(_, q: CallbackQuery):
    ...
    LOGGER(__name__).info(f"{user_id} removed all filter from {q.message.chat.id}")
    ...
    for trigger in actual_filters:
        ...
        if match:
            try:
                ...
                LOGGER(__name__).info(f"Replied with {msgtype} to {trigger} in {m.chat.id}")
            except Exception as ef:
