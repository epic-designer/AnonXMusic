...
from pyrogram.errors import FloodWait, PeerIdInvalid, MediaCaptionTooLong
 

import config as Config
from config import OWNER_ID as SUPPORT_STAFF,SUPPORT_CHAT, LOGGER_ID
...
from AnonXMusic.utils.functions import extract_user, extract_user_and_reason
from AnonXMusic.core.decorator.errors import capture_err
from AnonXMusic.utils.extras import StartPic
from AnonXMusic.supports import get_support_staff

SUPPORT_STAFF = get_support_staff()

__PLUGIN__ = "federation"

__buttons__ = [
    [
        ("𝙁𝙚𝙙 𝙤𝙬𝙣𝙚𝙧 𝙘𝙤𝙢𝙢𝙚𝙣𝙙𝙨", "federation.md_fed"),
        ("𝙁𝙚𝙙 𝙖𝙙𝙢𝙞𝙣 𝙘𝙤𝙢𝙢𝙚𝙣𝙙𝙨", "federation.fed_admin"),
    ],
    [("𝙐𝙨𝙚𝙧 𝙘𝙤𝙢𝙢𝙚𝙣𝙙𝙨", "federation.fed_user")],
]
...
@app.on_callback_query(filters.regex("^backf."))
async def send_mod_help(_, q: CallbackQuery):
    await q.edit_message_caption(
        caption=f"{__HELP__}",
        reply_markup=(await gen_federation_kb(q.message)),
    )
    await q.answer()
    return
    
@app.on_message(
    command(["federation", "fed"]) & filters.private,
)
async def markdownhelp(_, m: Message):
    await m.reply_photo(
        ...
        caption=f"{__HELP__}",
        quote=True,
        reply_markup=(await gen_federation_kb(m)),
    )
    LOGGER(__name__).info(f"{m.from_user.id} used cmd '{m.command}' in {m.chat.id}")
    return


@app.on_callback_query(filters.regex("^federation."))
async def get_federation_info(c: app, q: CallbackQuery):
    cmd = q.data.split(".")[1]
    kb = ikb([[("BACK", "backf.federation")]])
...
async def del_fed(client, message):
    ...
    if getinfo["owner_id"] == user.id or user.id not in SUPPORT_STAFF:
        fed_id = is_fed_id
    else:
...
@app.on_message(command("fedtransfer"))
@capture_err
...
@app.on_message(command("renamefed"))
@capture_err
...
@app.on_message(command(["setfedlog", "unsetfedlog"]))
@capture_err
async def fed_log(client, message):
    ...
    if member.status in [
        ...
        if info is False:
            return await message.reply_text("This federation does not exist.")
        if await is_user_fed_owner(fed_id, user.id):
            LOGGER_ID = LOGGER_ID if "/unsetfedlog" in message.text else chat.id
            loged = await set_log_chat(fed_id, LOGGER_ID)
...
@app.on_message(command("chatfed"))
@capture_err
...
@app.on_message(command("joinfed"))
@capture_err
async def join_fed(self, message):
    ...
    if (
        user.id in SUPPORT_STAFF or member.status != ChatMemberStatus.OWNER
    ) and user.id not in SUPPORT_STAFF:
    ...
    if len(args) > 1:
        ...
        if get_fedlog := getfed["LOGGER_ID"]:
            await app.send_message(
                get_fedlog,
...
@app.on_message(command("leavefed"))
@capture_err
async def leave_fed(client, message):
    ...
    member = await app.get_chat_member(chat.id, user.id)
    if member.status == ChatMemberStatus.OWNER or user.id in SUPPORT_STAFF:
        if await chat_leave_fed(int(chat.id)) is True:
            if get_fedlog := fed_info["LOGGER_ID"]:
                await app.send_message(
                    get_fedlog,
...
@app.on_message(command("fedchats"))
@capture_err
async def fed_chat(client, message):
    ...
    if len(args) > 1:
        ...
        all_admins = [fed_owner] + fed_admins + [int(Config.Config.BOT_ID)]
        if user.id not in all_admins and user.id not in SUPPORT_STAFF:
            return await message.reply_text(
...
@app.on_message(command("fedinfo"))
@capture_err
...
@app.on_message(command("fedadmins"))
@capture_err
...
@app.on_message(command("fpromote"))
@capture_err
async def fpromote(client, message):
    ...
    if await is_user_fed_owner(fed_id, user.id) or user.id in SUPPORT_STAFF:
...
@app.on_message(command("fdemote"))
@capture_err
...
@app.on_message(command(["fban", "sfban"]))
@capture_err
async def fban_user(client, message):
    ...
    fed_admins = info["fadmins"]
    fed_owner = info["owner_id"]
    all_admins = [fed_owner] + fed_admins + [int(Config.BOT_ID)]
    if from_user.id not in all_admins and from_user.id not in SUPPORT_STAFF:
        return await message.reply_text(
            "You need to be a Fed Admin to use this command"
        )
    if len(message.command) < 2:
    ...
    if not user_id:
        return await message.reply_text("I can't find that user.")
    if user_id in all_admins or user_id in SUPPORT_STAFF:
...
@app.on_message(command(["unfban", "sunfban"]))
@capture_err
...
@app.on_message(command("fedstat"))
...
@app.on_message(command("fbroadcast"))
@capture_err
