...
from AnonXMusic.database.locks_db import LOCKS
...
async def lock_perm(c: app, m: Message):
    ...
    if lock_type == "all":
        try:
            ...
            LOGGER(__name__).info(f"{m.from_user.id} locked all permissions in {m.chat.id}")
...
Use /locktypes to get the lock types"""
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} locked selected permissions in {m.chat.id}")
    except ChatNotModified:
        pass
    except ChatAdminRequired:
        await m.reply_text(text="I don't have permission to do that")
...
async def view_locks(_, m: Message):
    ...
    if v_perm is not None:
      ...
      """
            LOGGER(__name__).info(f"{m.from_user.id} used locks cmd in {m.chat.id}")
...
async def unlock_perm(c: app, m: Message):
    ...
    if unlock_type == "all":
        try:
            ...
            LOGGER(__name__).info(f"{m.from_user.id} unlocked all permissions in {m.chat.id}")
        except ChatNotModified:
            pass
        except ChatAdminRequired:
            await m.reply_text(text="I don't have permission to do that")
    ...
    try:
        LOGGER(__name__).info(f"{m.from_user.id} unlocked selected permissions in {m.chat.id}")
        await c.set_chat_permissions(
            chat_id,
            ChatPermissions(
...
@app.on_message(filters.group & ~filters.me,18)
...
async def prevent_approved(m: Message):
    ...
    for i in ul:
        try:
            ...
            continue
        LOGGER(__name__).info(f"Approved {i} in {m.chat.id}")
        await sleep(0.1)
    return
