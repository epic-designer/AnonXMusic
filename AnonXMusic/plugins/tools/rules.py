...
async def get_rules(_, m: Message):
    ...
    rules = db.get_rules()
    LOGGER(__name__).info(f"{m.from_user.id} fetched rules in {m.chat.id}")
...
async def set_rules(_, m: Message):
    ...
    db.set_rules(rules)
    LOGGER(__name__).info(f"{m.from_user.id} set rules in {m.chat.id}")
...
async def priv_rules(_, m: Message):
    ...
    if len(m.text.split()) == 2:
        ...
        if option in ("on", "yes"):
            ...
            LOGGER(__name__).info(f"{m.from_user.id} enabled privaterules in {m.chat.id}")
            msg = f"Private Rules have been turned <b>on</b> for chat <b>{m.chat.title}</b>"
        elif option in ("off", "no"):
            db.set_privrules(False)
            LOGGER(__name__).info(f"{m.from_user.id} disbaled privaterules in {m.chat.id}")
        ...
        LOGGER(__name__).info(f"{m.from_user.id} fetched privaterules preference in {m.chat.id}")
        await m.reply_text(msg)
    else:
...
async def clearrules_callback(_, q: CallbackQuery):
    ...
    LOGGER(__name__).info(f"{q.from_user.id} cleared rules in {q.message.chat.id}")
