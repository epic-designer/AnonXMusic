...
async def g_search(c: app, m: Message):
    ...
    try:
        ...
        await to_del.delete()
        await m.reply_text(f"Got an error:\nReport it at @{config.SUPPORT_CHANNEL}")
        LOGGER(__name__).error(e)
        LOGGER(__name__).error(format_exc())
        return


@app.on_message(command('anime'))
async def anime_search(c: app, m: Message):
    split = m.text.split(None, 1)
    if len(split) == 1:
        return await m.reply_text("No query given\nDo `/help search` to see how to use it")
...
@app.on_message(command('stack'))
async def stack_search(c: app, m: Message):
    split = m.text.split(None, 1)
    if len(split) == 1:
        return await m.reply_text("No query given\nDo `/help search` to see how to use it")
...
@app.on_message(command(["images","imgs"]))
