...
from AnonXMusic.utils.sticker_help import *
from AnonXMusic.utils.web_helpers import get_file_size
...
@app.on_message(command(["stickerinfo","stinfo"]))
...
@app.on_message(command(["stickerid","stid"]))
async def sticker_id_gib(c: app, m: Message):
    if not m.reply_to_message:
        await m.reply_text("Reply to a sticker")
        return
    elif not m.reply_to_message.sticker:
...
@app.on_message(command(["kang", "steal"]))
...
@app.on_message(command(["mmfb","mmfw","mmf"]))
async def memify_it(c: app, m: Message):
    if not m.reply_to_message:
    ...
    kb = IKM(
        [
            [
                IKB("Join for memes",url="https://t.me/umm_memes")
            ]
        ]
    )
    if len(m.command) == 1:
...
@app.on_message(command(["getsticker","getst"]))
async def get_sticker_from_file(c: app, m: Message):
