...
from AnonXMusic.utils.caching import (ADMIN_CACHE, TEMP_ADMIN_CACHE_BLOCK,
                                  admin_cache_reload)
...
async def adminlist_show(_, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"Adminlist cmd use in {m.chat.id} by {m.from_user.id}")
...
@app.on_message(command("zombies") & admin_filter)
...
async def reload_admins(_, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"Admincache cmd use in {m.chat.id} by {m.from_user.id}")
...
@app.on_message(command("fullpromote") & promote_filter)
...
@app.on_message(command("promote") & promote_filter)
...
@app.on_message(command("demote") & promote_filter)
async def demote_usr(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} demoted {user_id} in {m.chat.id}")
...
@app.on_message(command("invitelink"))
async def get_invitelink(c: app, m: Message):
    ...
    try:
        ...
        LOGGER(__name__).info(f"{m.from_user.id} exported invite link in {m.chat.id}")
    except ChatAdminRequired:
        await m.reply_text(text="I'm not admin or I don't have rights.")
...
@app.on_message(command("title") & admin_filter)
