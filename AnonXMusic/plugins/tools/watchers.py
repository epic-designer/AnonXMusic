...
from AnonXMusic.database.antispam_db import ANTISPAM_BANNED, GBan
from AnonXMusic.database.approve_db import Approve
from AnonXMusic.database.blacklist_db import Blacklist
from AnonXMusic.database.group_blacklist import BLACKLIST_CHATS
...
from AnonXMusic.utils.regex_utils import regex_searcher
...
async def antichanpin_cleanlinked(c: app, m: Message):
    try:
        msg_id = m.id
        ...
        if curr["antichannelpin"]:
            ...
            LOGGER(__name__).info(f"AntiChannelPin: msgid-{m.id} unpinned in {m.chat.id}")
        if curr["cleanlinked"]:
            await c.delete_messages(m.chat.id, msg_id)
            LOGGER(__name__).info(f"CleanLinked: msgid-{m.id} cleaned in {m.chat.id}")
...
@app.on_message(filters.user(list(ANTISPAM_BANNED)) & filters.group)
async def gban_watcher(c: app, m: Message):
    from AnonXMusic import SUPPORT_GROUP
    ...
    if _banned:
      ...
      To get unbanned, appeal at @{SUPPORT_GROUP}"""
            ...
            LOGGER(__name__).info(f"Banned user {m.from_user.id} in {m.chat.id} due to antispam")
            return
...
@app.on_message(filters.chat(BLACKLIST_CHATS))
async def bl_chats_watcher(c: app, m: Message):
    ...
    await c.send_message(
        ...
        (
            ...
            f"For Support, Join @{SUPPORT_CHAT}\n"
            "Now, I'm outta here!"
        ),
    )
    await c.leave_chat(m.chat.id)
    LOGGER(__name__).info(f"Joined and Left blacklisted chat {m.chat.id}")
    return
