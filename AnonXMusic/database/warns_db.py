...
class Warns(MongoDB):
    ...
    def __ensure_in_db(self, user_id: int):
        ...
        if not chat_data:
            ...
            LOGGER(__name__).info(f"Initialized Warn Document for {user_id} in {self.chat_id}")
            return new_data
        return chat_data
...
class WarnSettings(MongoDB):
    ...
    def __ensure_in_db(self):
        ...
        if not chat_data:
            ...
            self.insert_one(new_data)
            LOGGER(__name__).info(f"Initialized Warn Settings Document for {self.chat_id}")
            return new_data
        return chat_data
...
def __pre_req_warns():
    start = time()
    LOGGER(__name__).info("Starting Warns Database Repair...")
