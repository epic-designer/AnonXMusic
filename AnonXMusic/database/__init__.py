...
try:
    Powers_db_client = MongoClient(MONGO_DB_URI)
except PyMongoError as f:
    LOGGER(__name__).error(f"Error in Mongodb: {f}")
    exiter(1)
Powers_main_db = Powers_db_client[MONGO_DB_NAME]
...
def __connect_first():
    _ = MongoDB("test")
    LOGGER(__name__).info("Initialized Database!\n")
